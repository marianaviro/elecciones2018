...
<<<<<<< HEAD
  ...
  var yScale = d3
    ...
    //.domain([0, yMax])
    .range([
      dimensions.height - dimensions.margin_top,
  ...
  var xScale = d3
    ...
    //.domain(xDomain)
    .range([
      dimensions.margin_left + 30,
      dimensions.width - dimensions.margin_right
    ]);

  var parser = d3.timeParse('%Y-%m-%dT%H:%M:%S.%LZ');
  var first_day = parser('2017-12-02T18:00:00.477Z');
  var last_day = parser('2017-12-08T20:00:00.514Z');
...
>>>>>>> 7c5659bf32d61304a026f23711a34ae4917b176c
  ...
  var setupVis = function(rawData) {
    ...
    g
      ...
      .text(
        'Con base en más de 115.000 retweets de candidatos hechos en los últimos 7 días'
      );
    ...
    g
      ...
      .attr(
        'transform',
        `translate(${dimensions.margin_left},${dimensions.margin_top})`
      )
      .call(yAxis)
      .attr('opacity', 0);
    ...
    bars = bars
      ...
      .attr('height', (d, i) => {
        return (
          dimensions.height - yScale(d.cuenta_retweets) - dimensions.margin_top
        );
      })
      .attr('width', 10)
      .attr('opacity', 0);
    ...
    var tip = d3
      ...
      .html(
        d =>
          `<strong>${d.name} - ${d.twitter_handle} - ${
            d.cuenta_retweets
          } retweets</strong>`
      );
    g.call(tip);
  ...
  function countFollowing(candidate, edges) {
    return edges.filter(e => e.source.screenName === candidate.screenName)
      .length;
  }
  ...
  function countFollowers(candidate, edges) {
    return edges.filter(e => e.target.screenName === candidate.screenName)
      .length;
  }
    ...
    results[1].forEach(candidato => {
      ...
      for (var i = 0; i < candidato.growth.length; i++) {
        if (i === 0 || i === 1 || i === 2 || i === 3) {
          ...
          candidato.growth[i].cambio =
            candidato.growth[i].count - candidato.growth[i - 2].count;
      ...
      candidato.growth.forEach(fecha_seguidores => {
        fecha_seguidores.date = d3.timeParse('%Y-%m-%dT%H:%M:%S.%LZ')(
          fecha_seguidores.date
        );
      });
    });
