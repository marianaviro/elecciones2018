...
let scrollVis = function() {
	...
	//Y Axis
	var y = d3.scaleLinear().range([height - 400, 0]);
	...
	yAxis
		...
		.tickFormat(function(d, i, nodes) {
			return i < nodes.length - 1 ? '\xa0' + d : d + ' total retweets';
		});
	...
	var setupVis = function(rawData) {
		...
		g.append('i').text('de retweets por segundo.');
...
function display(data) {
	...
	scroll.on('active', function(index) {
		...
		d3.selectAll('.step .title').style('opacity', function(d, i) {
			return i === index ? 1 : 0;
		});
...
//API URL
var url = 'https://api_twitter.fabioespinosa.com/shady_tweets';

d3
	.queue()
	.defer(d3.json, url)
	.defer(d3.json, 'https://api_twitter.fabioespinosa.com/tweet/936590455856357376')
